...
func TestMaxOrderRatio(t *testing.T) {
	...
	priceBuy, _ := math.LegacyNewDecFromStr("1.1")
	priceSell, _ := math.LegacyNewDecFromStr("1.2")

	offerCoin := sdk.NewCoin(denomX, math.NewInt(1000))
	offerCoinY := sdk.NewCoin(denomY, math.NewInt(1000))
	...
	offerCoin = sdk.NewCoin(denomX, params.MinInitDepositAmount.Quo(math.NewInt(2)))
	offerCoinY = sdk.NewCoin(denomY, params.MinInitDepositAmount.Quo(math.NewInt(10)))
	app.SaveAccountWithFee(simapp, ctx, addrs[1], sdk.NewCoins(offerCoin), offerCoin)
	app.SaveAccountWithFee(simapp, ctx, addrs[2], sdk.NewCoins(offerCoinY), offerCoinY)
...
func TestOrderBookSort(t *testing.T) {
	...
	a, _ := math.LegacyNewDecFromStr("0.1")
	b, _ := math.LegacyNewDecFromStr("0.2")
	c, _ := math.LegacyNewDecFromStr("0.3")
	orderMap[a.String()] = types.Order{
		Price:        a,
		BuyOfferAmt:  math.ZeroInt(),
		SellOfferAmt: math.ZeroInt(),
	}
...
func TestExecutableAmt(t *testing.T) {
	...
	orderMap[a.String()] = types.Order{
		Price:        a,
		BuyOfferAmt:  math.ZeroInt(),
		SellOfferAmt: math.NewInt(30000000),
	}
	orderMap[b.String()] = types.Order{
		Price:        b,
		BuyOfferAmt:  math.NewInt(90000000),
		SellOfferAmt: math.ZeroInt(),
	}
	orderMap[c.String()] = types.Order{
		Price:        c,
		BuyOfferAmt:  math.NewInt(50000000),
		SellOfferAmt: math.ZeroInt(),
	}
	// make orderbook to sort orderMap
	orderBook := orderMap.SortOrderBook()
	...
	require.Equal(t, math.NewInt(140000000), executableBuyAmtX)
	require.Equal(t, math.NewInt(30000000), executableSellAmtY)
}

func TestPriceDirection(t *testing.T) {
	// increase case
	orderMap := make(types.OrderMap)
	a, _ := math.LegacyNewDecFromStr("1")
	b, _ := math.LegacyNewDecFromStr("1.1")
	c, _ := math.LegacyNewDecFromStr("1.2")
	orderMap[a.String()] = types.Order{
		Price:        a,
		BuyOfferAmt:  math.NewInt(40000000),
		SellOfferAmt: math.ZeroInt(),
	}
	orderMap[b.String()] = types.Order{
		Price:        b,
	...
	orderMap[c.String()] = types.Order{
		Price:        c,
		BuyOfferAmt:  math.ZeroInt(),
		SellOfferAmt: math.NewInt(20000000),
	}
	// make orderbook to sort orderMap
	orderBook := orderMap.SortOrderBook()
	poolPrice, _ := math.LegacyNewDecFromStr("1.0")
	result := orderBook.PriceDirection(poolPrice)
	require.Equal(t, types.Increasing, result)

	// decrease case
	orderMap = make(types.OrderMap)
	a, _ = math.LegacyNewDecFromStr("0.7")
	b, _ = math.LegacyNewDecFromStr("0.9")
	c, _ = math.LegacyNewDecFromStr("0.8")
	orderMap[a.String()] = types.Order{
		Price:        a,
		BuyOfferAmt:  math.NewInt(20000000),
		SellOfferAmt: math.ZeroInt(),
	}
	orderMap[b.String()] = types.Order{
		Price:        b,
		BuyOfferAmt:  math.ZeroInt(),
		SellOfferAmt: math.NewInt(40000000),
	}
	orderMap[c.String()] = types.Order{
		Price:        c,
		BuyOfferAmt:  math.NewInt(10000000),
		SellOfferAmt: math.ZeroInt(),
	}
	// make orderbook to sort orderMap
	orderBook = orderMap.SortOrderBook()
	...
	// stay case
	orderMap = make(types.OrderMap)
	a, _ = math.LegacyNewDecFromStr("1.0")

	orderMap[a.String()] = types.Order{
		Price:        a,
		BuyOfferAmt:  math.NewInt(50000000),
		SellOfferAmt: math.NewInt(50000000),
	}
	orderBook = orderMap.SortOrderBook()
	poolPrice, _ = math.LegacyNewDecFromStr("1.0")
	result = orderBook.PriceDirection(poolPrice)
...
func TestCalculateMatchStay(t *testing.T) {
	currentPrice := math.LegacyMustNewDecFromStr("1.0")
	orderBook := types.OrderBook{
		{Price: math.LegacyMustNewDecFromStr("1.0"), BuyOfferAmt: math.NewInt(5), SellOfferAmt: math.NewInt(7)},
	}
	require.Equal(t, types.Staying, orderBook.PriceDirection(currentPrice))
	r := orderBook.CalculateMatchStay(currentPrice)
	require.Equal(t, math.LegacyNewDec(5), r.EX)
	require.Equal(t, math.LegacyNewDec(5), r.EY)
}

// Match Stay case with fractional match type
func TestCalculateMatchStayEdgeCase(t *testing.T) {
	currentPrice, err := math.LegacyNewDecFromStr("1.844380246375231658")
	require.NoError(t, err)
...
func TestCalculateNoMatchEdgeCase(t *testing.T) {
	currentPrice, err := math.LegacyNewDecFromStr("1.007768598527187219")
	require.NoError(t, err)
	var orderBook types.OrderBook
...
func TestOrderbookValidate(t *testing.T) {
	for _, testCase := range []struct {
		...
		buyPrice := math.LegacyMustNewDecFromStr(testCase.buyPrice)
		sellPrice := math.LegacyMustNewDecFromStr(testCase.sellPrice)
		orderMap := types.OrderMap{
			buyPrice.String(): {
				Price:        buyPrice,
				BuyOfferAmt:  math.OneInt(),
				SellOfferAmt: math.ZeroInt(),
			},
			sellPrice.String(): {
				Price:        sellPrice,
				BuyOfferAmt:  math.ZeroInt(),
				SellOfferAmt: math.OneInt(),
			},
		}
		orderBook := orderMap.SortOrderBook()
		require.Equal(t, testCase.valid, orderBook.Validate(math.LegacyMustNewDecFromStr(testCase.currentPrice)))
	}
}
