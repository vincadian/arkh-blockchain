...
func TestGenesis(t *testing.T) {
	...
	require.Equal(t, math.LegacyNewDecWithPrec(3, 3), genState.Params.SwapFeeRate)

	// change swap fee rate
	params := lk.GetParams(ctx)
	params.SwapFeeRate = math.LegacyNewDecWithPrec(5, 3)

	// set params
	lk.SetParams(ctx, params)

	newGenState := lk.ExportGenesis(ctx)
	require.Equal(t, math.LegacyNewDecWithPrec(5, 3), newGenState.Params.SwapFeeRate)
...
func TestGenesisState(t *testing.T) {
	...
	params.PoolCreationFee = sdk.Coins{sdk.Coin{Denom: invalidDenom, Amount: math.NewInt(0)}}
	...
	X := math.NewInt(100_000_000)
	Y := math.NewInt(200_000_000)

	addrs := app.AddTestAddrsIncremental(simapp, ctx, 20, math.NewInt(10_000))
	...
	app.TestDepositPool(t, simapp, ctx, math.NewInt(30_000_000), math.NewInt(20_000_000), addrs[1:2], poolID, false)
	...
	require.Equal(t, math.NewInt(100_000), poolCoinBalance.Amount)
	...
	ctx3 := simapp3.BaseApp.NewContext(false)
	require.Panics(t, func() {
